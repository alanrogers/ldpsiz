...
To estimate population history parameters, {\ttfamily ldpsiz} minimizes the difference between values of $\sigma_d^2$ estimated from data and values predicted from population history. The predicted values are obtained from the recurrence equations either of Hill (1975, Theoretical Population Biology 8\+:117-\/126) or of Strobeck and Morgan (1978, Genetics 88\+:829-\/844). This calculation can be done in two different ways. The slow way (invoked using the {\ttfamily -\/-\/exact} option of \hyperlink{preld_8c}{preld}) iterates the difference equations one generation at a time. The default method speeds things up by solving differential equations that approximate the difference equations. These methods are fast enough to allow one to examine a large number of hypotheses about history parameters.
