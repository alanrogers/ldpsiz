 ...
 * like "ms2gtp my_input_file_name", it will allocate internal arrays
 ...
 * and 1161757 characters. So I ran `ms2gtp` like this:
 * 
...
void usage(void) {
    ...
    fprintf(stderr, " If no input file is given, ms2gtp reads stdin.");
    fprintf(stderr, " ms2gtp always writes\n  to standard output.\n");
...
int main(int argc, char **argv) {
    ...
    printf("# ms2gtp was run %s", ctime(&currtime));
    printf("# %-12s =", "cmd line");
    for(i = 0; i < argc; ++i)
        printf(" %s", argv[i]);
    putchar('\n');
